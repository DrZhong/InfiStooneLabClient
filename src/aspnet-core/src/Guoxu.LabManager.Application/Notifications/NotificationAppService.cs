...
using Abp.UI;
using Guoxu.LabManager.Notifications.Dto;

namespace Guoxu.LabManager.Notifications
{
    [AbpAuthorize]
    public class NotificationAppService : LabManagerAppServiceBase
    {
        ...
        public async Task<GetNotificationsOutput> GetUserNotifications(GetUserNotificationsInput input)
        {
            ...
            try
            {
                ...
                return new GetNotificationsOutput(totalCount, unreadCount, ObjectMapper.Map<List<UserNotificationDto>>(notifications));
            }
            catch (Exception )
            {
                 return new GetNotificationsOutput(0,0,new List<UserNotificationDto>());
            }
        }

        public async Task<UserNotificationDto> GetUserNotificationById(EntityDto<Guid> input)
        {
            var entity = await _userNotificationManager.GetUserNotificationAsync(AbpSession.TenantId, input.Id);
            return ObjectMapper.Map<UserNotificationDto>(entity);
        }
